...
class PoseNet(torch.nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.optimizer_posenet = torch.optim.Adam([dict(params=self.transNet.parameters(), lr=self.cam_lr),
                                                   dict(params=self.rotsNet.parameters(), lr=self.cam_lr * 0.2)])
...
class TransNet(torch.nn.Module):
    ...
    def define_network(self, cfg):
        ...
        self.batch_norms = nn.ModuleList([nn.BatchNorm1d(dim) for dim in cfg['layers_feat']])
    ...
    def forward(self, cfg, index):
        ...
        points_enc = torch.cat([index, points_enc], dim=-1)

        translation_feat = points_enc
        activ_f = getattr(torch_F, self.cfg['activ'])

        for li, (layer, bn) in enumerate(zip(self.mlp_transnet, self.batch_norms)):
            if li in cfg['skip']:
                translation_feat = torch.cat([translation_feat, points_enc], dim=-1)
            translation_feat = layer(translation_feat)
            translation_feat = bn(translation_feat)  # Apply Batch Normalization
    ...
    def positional_encoding(self, input, L):  # [B,...,N]
        shape = input.shape
        freq = 2 ** torch.arange(L, dtype=torch.float32, device=self.device) * math.pi
...
class RotsNet(torch.nn.Module):
    ...
    def forward(self, cfg, index):
        ...
        for li, (layer, bn) in enumerate(zip(self.mlp_quad, self.batch_norms)):
            if li in cfg['skip']:
                rotation_feat = torch.cat([rotation_feat, points_enc], dim=-1)
            rotation_feat = layer(rotation_feat)
            rotation_feat = bn(rotation_feat)  # Apply Batch Normalization
...
if __name__ == "__main__":
    ...
    random_se3 = 0.15 * torch.rand(1, 6)
