...
class CoSLAM():
    def __init__(self, config):
        ...
        self.oposenet = self.oposenets[0]
        self.iposenet = self.iposenets[0]
        self.lie = Lie()
    ...
    def global_CBA(self, batch, cur_frame_id):
        ...
        :return: None
        '''
        key_frame_ids_list = list(range(int(cur_frame_id // 50) * 50, cur_frame_id, self.config['mapping']['keyframe_every']))
        key_frame_ids = torch.tensor(key_frame_ids_list).to(self.device)
        not_key_frame_ids_list = [i for i in range(int(cur_frame_id // 50) * 50, cur_frame_id) if i not in key_frame_ids_list]
    ...
    def tracking_render(self, batch, frame_id, iters=None):
        ...
        if not self.config["tracking"]["use_continuous_pose"]:
            ...
            if frame_id % 50 != 0:
                flag = True
                former_frames_id = torch.range(int(frame_id // 50) * 50, int(frame_id - 1))
                est_former_c2ws = self.lie.se3_to_matrix(
                    self.lie.matrix_to_se3(self.oposenet.forward(torch.tensor(former_frames_id))) @ self.lie.matrix_to_se3(
                        self.iposenet.forward(torch.tensor(former_frames_id)))).detach().clone()
            ...
            if iters is None:
                if frame_id < 100:
                    ...
                    it = int(self.config['tracking']['iter'] * 1.5)
                elif frame_id < 300:
                    it = int(self.config['tracking']['iter'] * 2)
                elif frame_id < 400:
                    it = int(self.config['tracking']['iter'] * 2.5)
                else:
                    it = int(self.config['tracking']['iter'] * 3)
            else:
                it = iters
            for i in range(it):
                ...
                with torch.no_grad():
                    ...
                    if flag and iters is not None:
                        predicted_c2w = self.lie.se3_to_matrix(
                        self.lie.matrix_to_se3(self.oposenet.forward(torch.tensor(former_frames_id))) @ self.lie.matrix_to_se3(
                            self.iposenet.forward(torch.tensor(former_frames_id))))
                        former_loss = self.c2w_loss(predicted_c2w, est_former_c2ws)
                        loss += former_loss * 0.03
            ...
            # print(
            #     "Fucking loss: {}".format(self.lie.se3_to_matrix(
            #     self.lie.matrix_to_se3(self.oposenet.forward(torch.tensor(xxx))) @ self.lie.matrix_to_se3(
            #         self.iposenet.forward(torch.tensor(xxx)))).detach().clone(), est_former_c2ws[xxx])
            # )
    ...
    def c2w_loss(self, predicted, ground_truth):
        """
            计算c2w矩阵的损失

            参数：
            - predicted: 预测的c2w矩阵张量，形状为（n, 3, 4）
            - ground_truth: 真实的c2w矩阵张量，形状为（n, 3, 4）

            返回：
            - loss: 损失值
            """
        # 将矩阵张量展平为形状为（n, 12）的向量
        predicted_flat = predicted.view(predicted.size(0), -1)
        ground_truth_flat = ground_truth.view(ground_truth.size(0), -1)

        # 计算均方误差
        loss = F.mse_loss(predicted_flat, ground_truth_flat)
    ...
    def run(self):
        ...
        for i, batch in tqdm(enumerate(data_loader)):
            # self.oposenet = self.oposenets[i // 50]
            # self.iposenet = self.oposenets[i // 50]
            # Visualisation
            ...
            # Tracking + Mapping
                ...
                if i % self.config["training"]["posenet_every"] == 0:
                    ...
                    if i >= self.config["training"]["posenet_every"]:
                        self.oposenet = self.oposenets[i // 50]
                        self.iposenet = self.iposenets[i // 50]
