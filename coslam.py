...
class CoSLAM():
    def __init__(self, config):
        ...
        self.iposenet = PoseNet(self._config)  # ipoose_nets
        self.oposenet = PoseNet(self._config)  # opose_nets 2000 / 50 = 40
        self.num_posenet = int(2000 / self.config["training"]["posenet_every"]) # 2000 / 50
        self.num_posenets = self.config["training"]["posenet_every"]
        self.iposenets = [PoseNet(self._config) for i in range(self.num_posenet)]
        self.oposenets = [PoseNet(self._config) for i in range(self.num_posenet)]
        self.lie = Lie()
    ...
    def get_loss_from_ret(self, ret, tracking=True, rgb=True, sdf=True, depth=True, fs=True, use_continuous_pose=False,
        ...
        if use_continuous_pose and tracking:
            ...
            loss += self.config["training"]["o_posenet_weight"] * self.o_posenet_loss(self.oposenets[frame_id // self.num_posenets], frame_id)

            loss += self.config["training"]["i_posenet_weight"] * self.i_posenet_loss(self.iposenets[frame_id // self.num_posenets], frame_id)
    ...
    def first_every_frame_mapping(self, batch, i, niters=100):
        '''
                First frame mapping
                Params:
                    batch['c2w']: [1, 4, 4]
                ...
                '''
        print('First every frame mapping...')
        c2w = batch['c2w'][0].to(self.device)
        self.est_c2w_data[0] = c2w
        self.est_c2w_data_rel[0] = c2w
    ...
    def current_frame_mapping(self, batch, cur_frame_id):
        ...
        if not self.config["tracking"]["use_continuous_pose"]:
            ...
            c2w = self.lie.se3_to_matrix(
                self.lie.matrix_to_se3(self.oposenets[cur_frame_id // self.num_posenets].forward(torch.tensor(cur_frame_id))) @ self.lie.matrix_to_se3(
                    self.iposenets[cur_frame_id // self.num_posenets].forward(torch.tensor(cur_frame_id)))).squeeze().to(self.device).detach()
            print(cur_frame_id // self.num_posenets)
    ...
    def get_CBA_poses_loss(self, not_key_poses, frame_id):
        ...
        poses = self.lie.se3_to_matrix(
            self.lie.matrix_to_se3(self.oposenets[frame_id // self.num_posenets].forward(not_key_frame_ids)) @ self.lie.matrix_to_se3(
                self.iposenets[frame_id // self.num_posenets].forward(not_key_frame_ids))).to(self.device)
    ...
    def global_CBA(self, batch, cur_frame_id):
        ...
        poses_all = self.lie.se3_to_matrix(
            self.lie.matrix_to_se3(self.oposenets[cur_frame_id // self.num_posenets].forward(torch.tensor(range(cur_frame_id)))) @ self.lie.matrix_to_se3(
                self.iposenets[cur_frame_id // self.num_posenets].forward(torch.tensor(range(cur_frame_id)))))
        key_poses = self.lie.se3_to_matrix(
            self.lie.matrix_to_se3(self.oposenets[cur_frame_id // self.num_posenets].forward(key_frame_ids)) @ self.lie.matrix_to_se3(
                self.iposenets[cur_frame_id // self.num_posenets].forward(key_frame_ids))).to(self.device).detach()
        not_key_poses = self.lie.se3_to_matrix(
            self.lie.matrix_to_se3(self.oposenets[cur_frame_id // self.num_posenets].forward(not_key_frame_ids)) @ self.lie.matrix_to_se3(
                self.iposenets[cur_frame_id // self.num_posenets].forward(not_key_frame_ids))).to(self.device).detach()
        initial_not_key_poses = not_key_poses.detach().clone()
        ...
        for i in range(self.config['mapping']['iters']):
            ...
            loss += self.get_CBA_poses_loss(initial_not_key_poses, cur_frame_id)  # not_key_poses_loss
            ...
            if (i + 1) % cfg["mapping"]["pose_accum_step"] == 0:
                self.iposenets[i // self.num_posenets].step()
                self.oposenets[i // self.num_posenets].step()

        if len(key_frame_ids_list) > 1:
            for i in range(len(key_frame_ids[1:])):
                self.est_c2w_data[int(key_frame_ids[i + 1].item())] = self.lie.se3_to_matrix(
                    self.lie.matrix_to_se3(self.oposenets[i // self.num_posenets].forward(key_frame_ids[i + 1])) @ self.lie.matrix_to_se3(
                        self.iposenets[i // self.num_posenets].forward(key_frame_ids[i + 1]))).detach().clone()[0]

            if self.config['mapping']['optim_cur']:
                print('Update current pose')
                self.est_c2w_data[cur_frame_id] = self.lie.se3_to_matrix(
                    self.lie.matrix_to_se3(self.oposenets[i // self.num_posenets].forward(cur_frame_id)) @ self.lie.matrix_to_se3(
                        self.iposenets[i // self.num_posenets].forward(cur_frame_id))).detach().clone()[0]
    ...
    def tracking_render(self, batch, frame_id):
        ...
        if not self.config["tracking"]["use_continuous_pose"]:
            ...
            est_former_c2ws = self.lie.se3_to_matrix(
                self.lie.matrix_to_se3(self.oposenets[frame_id // self.num_posenets].forward(torch.tensor(former_frames_id))) @ self.lie.matrix_to_se3(
                    self.iposenets[frame_id // self.num_posenets].forward(torch.tensor(former_frames_id))))
            ...
            for i in range(self.config['tracking']['iter']):
                c2w_est = self.lie.se3_to_matrix(
                    self.lie.matrix_to_se3(self.oposenets[frame_id // self.num_posenets].forward(torch.tensor(frame_id))) @ self.lie.matrix_to_se3(
                        self.iposenets[frame_id // self.num_posenets].forward(torch.tensor(frame_id)))).to(self.device)

                print(frame_id // self.num_posenets)
                ...
                with torch.no_grad():
                    c2w_est = self.lie.se3_to_matrix(
                        self.lie.matrix_to_se3(self.oposenets[frame_id // self.num_posenets].forward(torch.tensor(frame_id))) @ self.lie.matrix_to_se3(
                            self.iposenets[frame_id // self.num_posenets].forward(torch.tensor(frame_id))))
            ...
            if frame_id % self.config['mapping']['keyframe_every'] != 0:
                ...
                c2w_key = self.lie.matrix_to_se3(
                    self.oposenets[i // self.num_posenets].forward(torch.tensor(kf_frame_id))) @ self.lie.matrix_to_se3(
                    self.iposenets[i // self.num_posenets].forward(torch.tensor(kf_frame_id)))
    ...
    def first_frame_pose_tracking(self, batch):
        ...
        for i in range(first_train_iters):
            iest_c2ws = self.iposenets[i // self.num_posenets].forward(times)
            oest_c2ws = self.oposenets[i // self.num_posenets].forward(times)
            est_c2ws = self.lie.se3_to_matrix(
            ...
            with torch.no_grad():
                ...
                for param in self.iposenets[i // self.num_posenets].parameters():
                    l1_penalty += torch.norm(param, p=1).to(self.device)
                for param in self.oposenets[i // self.num_posenets].parameters():
                    l1_penalty += torch.norm(param, p=1).to(self.device)
    ...
    def first_every_frame_pose_tracking(self, batch, i, iters):
        ...
        l1_lambda = 0.01
        c2w = self.lie.se3_to_matrix(
            self.lie.matrix_to_se3(self.oposenets[i // self.num_posenets].forward(k)) @ self.lie.matrix_to_se3(
                self.iposenets[i // self.num_posenets].forward(k))).to(self.device).detach()
        # train
